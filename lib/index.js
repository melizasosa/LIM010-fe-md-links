...
const functionMdLinksCli = (path, firtsOption, segundOption) => {
  ...
  if (path !== undefined && (firtsOption === '--validate' || firtsOption === '--v') && (segundOption === '--stats' || segundOption === '--s')) {
    result = mdLinks(path, {
      validate: true
    }).then(res => colors.blue(functionStatsAndValidate(res))).catch(() => `${path}: Ruta no existe`);
  } else if (path !== undefined && (firtsOption === '--validate' || firtsOption === '--v')) {
    result = mdLinks(path, {
      validate: true
    }).then(res => colors.blue(functionValidate(res))).catch(() => `${path}: Ruta no existe`);
  } else if (path !== undefined && (firtsOption === '--stats' || firtsOption === '--s')) {
    result = mdLinks(path, {
      validate: true
    }).then(res => colors.blue(functionStats(res))).catch(() => `${path}: Ruta no existe`);
  } else if (path !== undefined && firtsOption === undefined && segundOption === undefined) {
    result = mdLinks(path, {
      validate: false
    }).then(res => colors.blue(functionOnlyPath(res))).catch(() => `${path}: Ruta no existe`);
  } else {
    result = mdLinks(path, {
      validate: false
    }).catch(() => colors.red('Ruta no existe'));
  }
